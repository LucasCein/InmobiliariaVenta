...
const ItemList = ({ productos, home }) => {
    return (
        <Row md={'auto'} xl={'auto'} className={"w-100"}>
            {
                productos.length > 0 ? (
                    productos.map(({ id, descripcion, imagen, category, nombre, precio }) =>
                        <Col key={id} md={'auto'}>
                            <Item key={id} id={id} imagen={imagen} nombre={nombre} descripcion={descripcion} precio={precio}/>
                        </Col>
                    )
                ) : (
                    <Container className="text-center mt-5">
                        <Row>
                            <Col>
                                ...
                                <p className="text-muted">No pudimos encontrar lo que estás buscando. ¡Prueba con otra búsqueda!</p>
                            </Col>
                        </Row>
                    </Container>
                )
